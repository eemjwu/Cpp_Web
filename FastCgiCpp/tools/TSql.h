...
public:
	...
	void clearData(){m_Head.clear();m_vecColValueData.clear();m_vecKeyValueData.clear();};
	
private:

    std::vector< KeyValue > m_vecKeyValueData;
    std::vector< ColValue > m_vecColValueData;
	std::vector< std::string > m_Head;//表头
...
public:
	...
	TMysql()=default;//默认初始化
    ~TMysql();

	///@brief   初始化类
    ///@return  错误码
	int InitTMysql(const std::string sIp,
           unsigned short uiPort,
           const std::string sUserName,
           const std::string sPassword,
           const std::string sDatabase);
