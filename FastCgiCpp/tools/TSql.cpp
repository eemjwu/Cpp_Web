...
void TSqlData::pushHead(std::string s)
{
	m_Head.push_back(s);
}
...
std::vector<std::string> TSqlData::getHead()
{
	return m_Head;
}
...
int TMysql::InitTMysql(const std::string sIp,
           unsigned short uiPort,
           const std::string sUserName,
           const std::string sPassword,
           const std::string sDatabase)
...
{
	sqlData.clearData();//清除sqlData中之前的数据
    if(!m_pMySql)
    {
        std::cout << "sql is disconnnect!" << std::endl;
        return MYSQL_E_CONNECT;
	...
	if (result == 0) return MYSQL_SUCCESS; // 执行了不返回任何信息的语句  防止出现bug  wmj添加
    ...
    if(result)
    {
        ...
        for(int row = 0; row < num_row; row++)
        {
            ...
            {
				// bug 修正：当查询的字段为空
				if (mysqlRow[col] == NULL || mysqlRow[col] == "") {
					
					colValue[col] = "NULL";
					keyValue[field[col].name] = "NULL";
					continue;
				}
				//
			...
			if (row == 0)
			{
				for (int col = 0; col < num_col; col++)
				{
					sqlData.pushHead(field[col].name);
...
int TMysql::is_connvalid()
{
	if(mysql_ping(m_pMySql) != 0)
	{
		return MYSQL_E_CONNVALID;
	}
	return MYSQL_SUCCESS;
...
//#if 1
    ...
    WebTool::TMysql mysql("localhost", 3306, "root",\
                          "123456", "web");
    mysql.openDB();
   ...
   // mysql.queryMySql(sqlStr);
    ...
    mysql.execSQL("select * from t_file_pic_info where user_id = '001'", sqlData); 
    CDBG << sqlData.getCol();
    CDBG << sqlData.getRow();
    CDBG << sqlData.getData(0, "remark");
    CDBG << sqlData.getData(0, 1);
    std::cout << std::endl;
	std::vector<std::string> Head = sqlData.getHead();
	CDBG << Head[0] <<" "<< Head[1] << " " << Head[2] << std::endl;
    mysql.closeDB();
